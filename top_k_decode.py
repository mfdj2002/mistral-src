...
def sample(logits: torch.Tensor, temperature: float, top_p: float, k: int):
    if temperature > 0:
...
def cot_decoding(logits: torch.Tensor, k: int):
    pass
...
def generate(prompts: List[str], model: Transformer, tokenizer: Tokenizer, *, max_tokens: int,  temperature: float, chunk_size: int = None):
    ...
    # # Encode prompt without chunking
    # assert all(len(p) > 0 for p in encoded_prompts)
    # prelogits = model.forward(
    #     torch.tensor(encoded_prompts, device=model.device, dtype=torch.long),
    ...
    #     # Extend logprobs with logits for each token in the sequence
    #     # Here, we're assuming the logits are indexed [sequence, token]
    #     # Adjust the indexing according to your actual data structure and requirements
    #     logprobs[i_seq].extend([logits[i_seq, i].item() for i in range(len(sequence))])
    ...
    for i_token in range(max_tokens):
        next_token = sample(last_token_prelogits, temperature=temperature, top_p=0.8, k=10)
...
def main(
    model_path: str, max_tokens: int = 512, temperature: float = 0.7, num_pipeline_ranks=1
):
    ...
    transformer = Transformer.from_folder(
        Path(model_path), max_batch_size=16, num_pipeline_ranks=num_pipeline_ranks
    )
